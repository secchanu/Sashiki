...
use gpui::{
    actions, div, prelude::*, rgb, rgba, App, AsyncApp, Bounds, Context, Element, ElementId,
    ElementInputHandler, Entity, EntityInputHandler, FocusHandle, Focusable, GlobalElementId,
    Hsla, InspectorElementId, IntoElement, KeyBinding, LayoutId, MouseButton, ParentElement,
    Pixels, Render, Styled, UTF16Selection, WeakEntity, Window,
};
use std::ops::Range;
use std::sync::Arc;
...
/// Terminal row: (cells, is_cursor_line)
type TerminalRow = (Vec<TerminalCell>, bool);
...
impl TerminalView {
    ...
    pub fn new_with_directory(working_directory: std::path::PathBuf, cx: &mut Context<Self>) -> Self {
    ...
    fn new_internal(working_directory: Option<std::path::PathBuf>, cx: &mut Context<Self>) -> Self {
        let (terminal, event_rx) =
            Terminal::new(working_directory).expect("Failed to create terminal");
    ...
    fn render_terminal_content(&self, _cx: &mut Context<Self>) -> impl IntoElement {
        let mut rows: Vec<TerminalRow> = Vec::new();
        ...
        self.terminal.with_term(|term| {
            ...
            for line_idx in 0..total_lines {
                ...
                rows.push((row_cells, is_cursor_line));
            }
        });
        ...
        div()
            ...
            .text_sm()
            .children(rows.into_iter().map(|(row_cells, _is_cursor_line)| {
                div()
                    .flex()
                    .flex_row()
                    .children(row_cells.into_iter().map(|(c, fg, bg, is_cursor)| {
                        let mut cell_div = div()
                            .text_color(if is_cursor { Hsla::from(rgb(BG_BASE)) } else { fg });
